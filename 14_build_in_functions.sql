...
select format(12113123132121.5456454654,3);

-- insert(기준 문자열, 위치, 길이, 삽입할 문자열)
select insert ('내 이름은 아무개입니다.', 7, 3, '정고은');

-- left, right
-- left : 왼쪽에서 문자열의 길이만큼 반환
...
-- lpad: 문자열을 길이만 왼쪽으로 늘린 후에 빈 곳을 문자열로 채운다.
-- rpad: 문자열을 길이만 오른쪽으로 늘린 후에 빈 곳을 문자열로 채운다.
-- 990624-2******
...
-- floor(숫자) : 버림값 반환
-- round(숫자) : 반올림값 반환 
select ceiling(1234.12), floor(1234.56), round(1234.36);

-- mod(숫자1, 숫자2) or 숫자1 % 숫자2 or 숫자1 mod 숫자2
-- 숫자1 을 숫자2 로 나눈 나머지 추출
...
-- addtime(날짜/시간, 시간)
-- subtime(날짜/시간, 시간)
select addtime('2024-03-04 17:42:00', '00:30:00');
select subtime('2024-03-04 17:42:00', '00:30:00');
...
-- 연 월 일 시 분 초 밀리초 
select year(curdate()), month(curdate()), day(curdate()), second(curtime()), microsecond(curtime());

-- datediff(날짜1, 날짜2)
-- timediff(날찌1 or 시간 1, 날짜2 또는 시간2)
