...
-- select 쿼리문을 저장한 객체로 가상케이블 이라고 불린다. 
-- 실질적인 데이터를 물리적으로 저장하지 않고, 퀴리만 저장했지만
...
-- view 의 결과도 같이 변경된다. 
insert into tbl_menu values (null, '숙취해소제', 5500, 4, 'Y');
select * from tbl_menu;
select * from hansik;
...
-- 2. 뷰에 포함되지 않은 컬럼 중 베이스 텡이블 컬럼이 not null 제약조건이 지정된 경우
-- 3. 산술표현식이 정의된 경우
