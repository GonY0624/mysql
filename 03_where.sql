...
-- tbl_menu 테이블에서 메뉴 이름, 가격, 판매 상태를 조회할 건데
...
-- tbl_menu 테이블에서 메뉴 코드, 이름, 판매 상태 를 조회할건데
...
-- where 절과 함께 사용을 하며
-- 자바랑 똑같다. 
-- or 연산자  : 둘 다 null 이 아니면서 하나라도 0이 아닌 값이 있을 경우 1을 반환한다. 
...
-- 가격이 5000원이 넘거나 카테고리 코드가 10인 녀석들 조회해줘
select *
from tbl_menu
where
	menu_price >= 5000
...
-- tbl_menu 테이블에서 모든 칼럼을 조회하는데
-- 판매 상태가 y 이면서 카테고리 코드가 10인 녀석들만 조회해줘
select *
from tbl_menu
where
	orderable_status = 'Y'
...
-- 따라서 or 의 운선순위를 높이고 싶으면 소괄호를 사용한다. 
select
	*
from
	tbl_menu
...
-- 부정 between(between 사이의 숫자 외의 것들을 출력)
select
	menu_name,
    menu_price,
    category_code
...
-- like 연산자
-- tbl_menu 테이블에서 메뉴 이름과 가격을 조회하는데
...
-- in 연산자 활용
-- tbl_menu 테이블 메뉴이름, 카테고리 코드 조회하는데 
