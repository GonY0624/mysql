...
-- 테이블의 컬럼 설정 방법
-- 컬럼이름 데이터타입(길이) [not null] [defaulty value] [auto_increment] colum_contraint;
...
-- tb2 테이블에 col2 라는 컬럼 추가 (int 형, not null 제약 조건 존재)
alter table tb2
...
-- auto_increment 가 걸려있는 컬럼은 primary key 제거가 안 된다. 
...
-- 0 으로 채워진 date 컬럼이 존재하면 안 되기 때문이다. 
